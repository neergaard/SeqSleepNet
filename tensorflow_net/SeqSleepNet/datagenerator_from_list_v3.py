...
class DataGenerator3:
    def __init__(self, list_of_files, file_sizes, data_shape=np.array([29, 129]), seq_len=20):
    ...
    def read_mat_filelist(self):
        ...
        for i in range(len(self.list_of_files)):
            X, y, label = self.read_mat_file(self.list_of_files[i].strip())
    ...
    def normalize(self, meanX, stdX):
        # data normalization for time-frequency input here
        X = self.X
        X = np.reshape(X, (self.data_size*self.data_shape[0], self.data_shape[1]))
        X = (X - meanX) / stdX
        self.X = np.reshape(X, (self.data_size, self.data_shape[0], self.data_shape[1]))
